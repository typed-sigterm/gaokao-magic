...
const props = withDefaults(
  defineProps<Partial<PortalParams>>(),
  {
    portalComplexity: 4,
    crystalCount: 12,
    primaryColor: '#9b59b6',
...
function initThreeJS() {
  ...
  const lightColors = [params.value.primaryColor, params.value.secondaryColor, params.value.accentColor, params.value.vortexColor];
  for (let i = 0; i < 6; i++) {
  ...
  renderer = new THREE.WebGLRenderer({ canvas: canvasRef.value, antialias: true, powerPreference: 'high-performance' });
  ...
  bloomPass = new UnrealBloomPass(new THREE.Vector2(width, height), params.value.bloomStrength, params.value.bloomRadius, params.value.bloomThreshold);
...
function addPortalShader(material: any) {
  ...
  material.onBeforeCompile = (shader: any) => {
    ...
    shader.fragmentShader = `
      uniform float time; uniform float pulseTime; uniform float portalSpeed;
      uniform vec3 portalColor; uniform float dimensionShift;
...
function createCosmicBackground() {
  ...
  const mat = new THREE.PointsMaterial({ size: 0.3, vertexColors: true, sizeAttenuation: true, blending: THREE.AdditiveBlending, depthWrite: false, transparent: true });
  const stars = new THREE.Points(geo, mat);
  scene.add(stars); meshes.push(stars); materials.push(mat);
}
...
function createPortalCore() {
  ...
  const mat = new THREE.ShaderMaterial({
    ...
    vertexShader: `
      uniform float time; uniform float dimensionShift; varying vec3 vPos; varying vec3 vNorm;
      void main() {
    ...
    fragmentShader: `
      uniform float time; uniform float pulseTime; uniform float dimensionShift;
      ...
      void main() {
        ...
        if(timeSincePortal > 0.0 && timeSincePortal < 1.0) {
          float burst = 1.0 - timeSincePortal; finalColor += vec3(1.0) * burst * 3.0;
        }
...
function createDimensionalStreams() {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    scene.add(stream); meshes.push(stream);
  }
}
...
function createEnergyParticles() {
  ...
  geo.setAttribute('position', new THREE.BufferAttribute(positions, 3));
  const mat = new THREE.PointsMaterial({ size: 0.08, color: params.value.vortexColor, blending: THREE.AdditiveBlending, transparent: true, opacity: 0.8 });
  const particles = new THREE.Points(geo, mat);
  scene.add(particles); meshes.push(particles); materials.push(mat);
}
...
function createSpaceDistortion() {
  ...
  const mat = new THREE.ShaderMaterial({
    ...
    vertexShader: `
      uniform float time; uniform float dimensionShift; varying vec3 vNorm; varying vec3 vPos;
      void main() { vNorm = normal; vPos = position; gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0); }`,
    fragmentShader: `
      uniform float time; uniform float dimensionShift; uniform vec3 color1; uniform vec3 color2;
  ...
  scene.add(distortion); meshes.push(distortion); materials.push(mat);
}

function createPortalScene() {
  meshes.forEach(mesh => scene.remove(mesh));
  materials.forEach(mat => mat.dispose());
  meshes = []; materials = []; portalMaterials = [];
...
function handleResize() {
  if (!camera || !renderer || !composer || !canvasRef.value)
    return;
  const container = canvasRef.value.parentElement;
  if (!container)
    return;
...
function animate() {
  ...
  // smooth camera zoom towards targetZ
  if (camera) {
  ...
  portalLights.forEach((light, i) => {
    ...
    const r = 10 + Math.sin(time * 0.5 + i) * 3;
    light.position.x = Math.cos(angle) * r;
    light.position.z = Math.sin(angle) * r;
  ...
  meshes.forEach((mesh, i) => {
    ...
    const m = mesh as THREE.Points;
    if (m.material && (m.material as THREE.PointsMaterial).type === 'PointsMaterial') {
      const pos = m.geometry.attributes.position.array as Float32Array;
      for (let j = 0; j < pos.length; j += 3) {
        pos[j] += Math.sin(time + j) * 0.001;
        pos[j + 1] += Math.cos(time + j) * 0.001;
        pos[j + 2] += Math.sin(time * 0.7 + j) * 0.001;
      }
...
// Exposed: zoom camera in (click effect)
function zoomIn() {
  targetZ = Math.max(8, targetZ - 2);
}
...
<template>
  <div style="position:relative;height:100%;width:100%;overflow:hidden">
    <canvas ref="canvasRef" style="position:absolute;inset:0;display:block;height:100%;width:100%" />
  </div>
</template>
